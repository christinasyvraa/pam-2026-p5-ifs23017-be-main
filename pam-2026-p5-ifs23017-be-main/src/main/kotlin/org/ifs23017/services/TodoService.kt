...
import org.ifs23017.data.DataResponse
import org.ifs23017.data.TodoListMeta
import org.ifs23017.data.TodoListResponse
import org.ifs23017.data.TodoRequest
import org.ifs23017.helpers.ServiceHelper
import org.ifs23017.helpers.ValidatorHelper
import org.ifs23017.repositories.ITodoRepository
...
class TodoService(
    ...
    suspend fun getAll(call: ApplicationCall) {
        ...
        if (page < 1) {
            throw AppException(400, "Parameter page minimal bernilai 1")
        }
        if (perPage < 1) {
            throw AppException(400, "Parameter perPage minimal bernilai 1")
        }

        val offset = (page - 1L) * perPage
        val total = todoRepo.countAll(user.id, search)
        val totalPages = if (total == 0L) 0 else ((total + perPage - 1) / perPage).toInt()
        val todos = todoRepo.getAll(user.id, search, perPage, offset)
    ...
    suspend fun post(call: ApplicationCall) {
        ...
        validator.validate()
        if (!request.isUrgencyValid()) {
            throw AppException(400, "Nilai urgency tidak valid! Gunakan HIGH, MEDIUM, atau LOW.")
        }
    ...
    suspend fun put(call: ApplicationCall) {
        ...
        if (request.urgency.isNullOrBlank()) {
            request.urgency = oldTodo.urgency
        } else if (!request.isUrgencyValid()) {
            throw AppException(400, "Nilai urgency tidak valid! Gunakan HIGH, MEDIUM, atau LOW.")
        }
